-- we use table materialization for now; incremental loading can be added as data volume grows
...
, token_mapping AS (
    ...
    SELECT * FROM {{ ref('native_token_mapping') }}
...
SELECT
    ...
    (CAST(transfers.min_amount_raw AS DOUBLE)
        / POW(10, COALESCE(usd_prices.decimals, 18))
