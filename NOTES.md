...
- **Modular ingestion script** — currently `duckdb_creation.py` is a single file with all three loaders; with more data sources it should be split into separate modules per source (e.g. `loaders/transfers.py`, `loaders/prices.py`, `loaders/fx_rates.py`) with a shared config and a CLI entrypoint
